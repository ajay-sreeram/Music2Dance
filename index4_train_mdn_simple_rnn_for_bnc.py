...
class PoseMusicDataset_new(Dataset):
    ...
    def __init__(self, bundle_len = 1000, seq_len = 100, init_step = 0,  prev_poses_cnt = 5):
        ...
        self.classical_data_files_path = "data/processed_30fps_only_inout_c"
        
        self.seq_samples_cnt = 1600 #1590 #790 #1750
        ...
        self.init_files_names_array()
        
        # better to have multiples of 10
    def init_files_names_array(self):
        for root, _, files in os.walk(self.data_files_path):#processed_30fps #tmp
            for file in files:
                if file.endswith(".json"):
                    self.data_files_names.append(self.data_files_path+"/"+file)
        
        for root, _, files in os.walk(self.classical_data_files_path):#processed_30fps #tmp
            for file in files:
                if file.endswith(".json"):
                    self.data_files_names.append(self.classical_data_files_path+"/"+file)
        
        random.shuffle(self.data_files_names)
    ...
    def __getitem__(self, idx):
        ...
        cur_data_file = self.data_files_names[int(bundle_min/1000)]
        
        if '_c_' in cur_data_file:
            is_classical = True
        else:
            is_classical = False
        ...
        with open(cur_data_file) as f:
            ...
            for index in range(10):
                ...
                if is_classical == True:
                    cur_pose_style =  np.concatenate([np.ones((self.seq_len,1)), np.zeros((self.seq_len,1))], 1)
                else:
                    cur_pose_style =  np.concatenate([np.zeros((self.seq_len,1)), np.ones((self.seq_len,1))], 1)
                ...
                if self.last_n_poses is None:
                    ...
                    prev_pose_style =  np.concatenate([np.zeros((self.prev_poses_cnt,1)), np.zeros((self.prev_poses_cnt,1))], 1)
                else:
                    prev_poses_input = self.last_n_poses[:-1]
                    prev_poses_target = self.last_n_poses[1:]
                    prev_pose_style = self.last_n_poses_style
                                
                sample = {
                    'audio_inputs': audio_inputs,
                    'prev_poses': prev_poses_input,  #np.zeros(34, dtype=np.float32),
                    'next_steps': np.concatenate((prev_poses_target, next_steps), 0),
                    'pose_styles': np.concatenate((prev_pose_style, cur_pose_style), 0)
                }
                ...
                self.last_n_poses_style = sample['pose_styles'][-self.prev_poses_cnt:]
...
output_dir = "output/motiondance_simplernn_bnc"
...
writer = SummaryWriter(comment='mdn_simple_rnn_gpu_bnc_{0}'.format(epochs))
...
class MDNRNN(torch.nn.Module):
    def __init__(self, dim, cnnEncoder, z_size, n_hidden=256, n_gaussians=5, n_layers=1, pose_type_cnt=2):
        ...
        self.lstm = torch.nn.LSTM(dim + pose_type_cnt, n_hidden, n_layers, batch_first=True)
    ...
    def forward(self, audio_inputs, prev_steps, pose_type_inputs, h):
        # Forward propagate LSTM
        x = []
        pose_type_inputs = pose_type_inputs.chunk(pose_type_inputs.size(1), dim=1)
        
        for i, input_t in enumerate(prev_steps.chunk(prev_steps.size(1), dim=1)):
            p_steps = self.prev_steps_fc(input_t)
            p_steps = p_steps.view((p_steps.size(0), -1))
            p_steps_type = pose_type_inputs[i].view((pose_type_inputs[i].size(0), -1))
            p_steps = torch.cat((p_steps, p_steps_type), 1)
            x += [p_steps]
        ...
        for i, input_t in enumerate(audio_inputs.chunk(audio_inputs.size(1), dim=1)):
            ...
            p_steps_type = pose_type_inputs[i].view((pose_type_inputs[i].size(0), -1))
            h_ = torch.cat((h_, p_steps_type), 1)            
            x += [h_]
...
n_gaussians = 5
n_layers = 2
pose_type_cnt = 2 #ballet and classical

gpu_cnt = torch.cuda.device_count()
if gpu_cnt == 1:
    sys.stdout.write("One GPU\n")
    model = MDNRNN(audio_convout_size, CNNFeat, z_size, n_hidden, n_gaussians, n_layers, pose_type_cnt).cuda()
elif gpu_cnt > 1:
    sys.stdout.write("More GPU's: {0}\n".format(gpu_cnt))
    model = torch.nn.DataParellel( MDNRNN(audio_convout_size, CNNFeat, z_size, n_hidden, n_gaussians, n_layers, pose_type_cnt).cuda() )
else:
    sys.stdout.write("No GPU\n")
    model = MDNRNN(audio_convout_size, CNNFeat, z_size, n_hidden, n_gaussians, n_layers, pose_type_cnt)
...
# load previous check point

#model_saved_path = "output/motiondance_simplernn_bnc/checkpoints/epoch_100.pth.tar"
...
for epoch in range(epochs):
    ...
    for i, data in enumerate(dataloader):
        ...
        pose_styles = data['pose_styles'].type(torch.cuda.DoubleTensor)
        ...
        (pi, mu, sigma), hidden = model(audio_inputs, prev_steps, pose_styles, hidden)        
    ...
    if(epoch+1)%save_checkpoint_distance == 0:
...
final_freezed_path = output_dir+"/frozen/final_model_300_plus_{0}.pt".format(epochs)
